...
export const Route = createFileRoute("/history")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your past Audit.ai reports." },
    ],
  }),
    ...
    <div className="min-h-screen bg-white text-neutral-900">
      ...
      <main className="mx-auto max-w-4xl px-6 py-12">
        <div className="flex items-baseline justify-between">
          <h1 className="text-3xl font-semibold tracking-tight">My audits</h1>
          <Link
            to="/audit"
        ...
        {audits === null && !error ? (
          ...
          <div className="mt-12 rounded-lg border border-dashed border-neutral-200 p-10 text-center">
            <p className="text-sm text-neutral-600">No audits yet.</p>
            <Link
              to="/audit"
          ...
          <ul className="mt-8 space-y-3">
            {audits?.map((a) => (
              <li
                key={a.id}
                className="flex items-center justify-between rounded-lg border border-neutral-200 p-5 transition-colors hover:border-neutral-300"
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-neutral-500">
                    ...
                    <span>{a.issueCount} issue{a.issueCount === 1 ? "" : "s"}</span>
                  </div>
                </div>
                <div className="ml-6 flex items-center gap-4">
                  <div className="text-right">
                    <div className="text-2xl font-bold tabular-nums">{a.score}</div>
                    <div className="text-[10px] uppercase tracking-wide text-neutral-400">/ 100</div>
                  </div>
                  <Link
                    to="/results/$id"
                    params={{ id: a.id }}
